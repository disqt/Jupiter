...
function WalkingWorkoutForm() {
  ...
  const storageKey = workoutId ? `walking-edit-${workoutId}` : `walking-draft-${date}`;
  ...
  useEffect(() => {
    ...
    fetchWorkout(parseInt(workoutId)).then((data) => {
      ...
      if (wd) {
        ...
        if (wd.distance) setDistance(String(wd.distance));
      }
  ...
  useEffect(() => {
    ...
    const data = { duration, distance, customEmoji, customName };
    const hasData = duration || distance;
    if (hasData) {
      localStorage.setItem(storageKey, JSON.stringify(data));
    } else {
      localStorage.removeItem(storageKey);
    }
  }, [duration, distance, customEmoji, customName, storageKey, date, workoutId, editing]);
  ...
  return (
    <div className="page-container px-5 pb-36 lg:pb-20">
      <WorkoutFormHeader
        emoji={customEmoji || WORKOUT_CONFIG.marche.defaultEmoji}
        name={customName || t.walkingWorkout}
        defaultName={t.walkingWorkout}
      ...
      <div className="md:grid md:grid-cols-2 md:gap-4">
        ...
        <div className="mb-4">
          ...
          <input type="text" inputMode="decimal" value={distance} onChange={(e) => { if (/^[0-9]*\.?[0-9]*$/.test(e.target.value)) setDistance(e.target.value); }} placeholder="5.0"
            disabled={!!workoutId && !editing}
            className={`w-full py-3.5 px-4 bg-bg-card border border-border rounded-sm text-text font-inherit text-[15px] outline-none transition-colors duration-200 focus:border-accent placeholder:text-text-muted ${workoutId && !editing ? 'opacity-50 cursor-not-allowed' : ''}`} />
        </div>
      </div>
      ...
      {(!workoutId || editing) && (
        <button onClick={handleSave} disabled={saving}
          className="w-full py-4 border-none rounded-card font-inherit text-[15px] font-semibold cursor-pointer mt-6 transition-all duration-200 active:scale-[0.98] bg-walking text-white shadow-[0_4px_20px_rgba(245,158,11,0.3)] disabled:opacity-50 disabled:cursor-not-allowed tracking-wide">
