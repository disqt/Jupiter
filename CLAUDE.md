...
- Onboarding: `OnboardingFlow` overlay shown when `has_seen_onboarding` is false on user record. 6 button-navigated screens (welcome → goal → medals → sports → calendar → ready). `SwipeContainer` handles slide transitions (no swipe — button-only navigation via `goNext`/`goBack`). `OnboardingGate` in Providers.tsx checks auth state and renders overlay (must also render `SplashDismiss` to dismiss splash). Goal screen passes selected value to medal screen for dynamic example. Completion calls `PUT /api/auth/me` with `has_seen_onboarding: true`. Register page at `/register` (full-screen, 2-step: identity → password). Guest data migration happens after registration. Login API returns `has_seen_onboarding` in response; both login() and register() trigger onboarding in AuthProvider.
- WeeklyProgress: fetches once on mount only (no `pathname` dep) — do NOT add pathname to deps, it causes a refetch on every navigation.
